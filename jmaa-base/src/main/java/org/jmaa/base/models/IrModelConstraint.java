 ...
 * @author Eric Liang
 */
@Model.Meta(name = "ir.model.constraint", label = "模型约束", authModel = "ir.model")
...
public class IrModelConstraint extends Model {
    ...
    static Field model_id = Field.Many2one("ir.model").label("模型").ondelete(DeleteMode.Cascade);
    static Field module_id = Field.Many2one("ir.module").label("模块").ondelete(DeleteMode.Cascade);
    ...
    public void reflectModels(Records rec, Collection<String> models, String module) {
        ...
        for (String model : models) {
            ...
            if(meta.isAuto()){
                for (org.jmaa.sdk.core.UniqueConstraint uc : meta.getUniques()) {
