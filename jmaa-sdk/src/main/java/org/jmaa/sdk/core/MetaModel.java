...
import org.jmaa.sdk.fields.DateTimeField;
import org.jmaa.sdk.fields.IdField;
...
public class MetaModel {
     ...
     * 一对多，多对多的相反字段
     */
    Map<String, List<String>> fieldInverses = new HashMap<>();
    /**
     * 模型的方法
     */
     ...
     * 方法约束
     */
    Map<String, Set<String>> constrains;
    /**
     * 方法约束
     */
    List<Tuple<String, Boolean>> onDelete;
    /**
     * 变更方法
     */
    Map<String, Set<String>> onChanges;
    Set<String> actions;
    /**
     * 是否自定义的模型
     */
     ...
     * 授权的子模型
     */
    List<String> authChildren = new ArrayList<>();
    /**
     * 数据库表名
     */
    String table;
    ...
    public List<String> getFieldInverses(String field) {
        return fieldInverses.get(field);
    }
     ...
     * @return
     */
    public Collection<String> getConstrains(String field) {
        List<String> result = new ArrayList<>();
        Collection<String> methods = constrains.get(field);
        if (methods != null) {
            result.addAll(methods);
        }
        methods = constrains.get("@all");
        if (methods != null) {
            result.addAll(methods);
        }
        return result;
     ...
     * @return
     */
    public Collection<String> getOnChanges(String field) {
        List<String> result = new ArrayList<>();
        Collection<String> methods = onChanges.get(field);
        if (methods != null) {
            result.addAll(methods);
        }
        methods = onChanges.get("@all");
        if (methods != null) {
            result.addAll(methods);
        }
        return result;
     ...
     * @return
     */
    public List<String> getAuthChildren() {
        return authChildren;
    }
    ...
    public Records browse(Environment env, String[] ids, Supplier<String[]> prefetchIds) {
        return new Records(env, this, ids, prefetchIds);
    }

    public Records browse(Environment env) {
        return new Records(env, this, ArrayUtils.EMPTY_STRING_ARRAY, null);
    }
     ...
     * @return
     */
    public boolean hasBase(String model) {
        for (MetaModel base : bases) {
            if (base.getName().equals(model)) {
                return true;
            }
        }
        return false;
     ...
     * @return
     */
    public Object invoke(Records rec, String method, Object[] args) {
        if (methods.containsKey(method)) {
            for (MetaMethod metaMethod : methods.get(method)) {
                ModelInterceptor.INVOKE_CURRENT.set(true);
                return metaMethod.invoke(rec, args);
            }
        }
        String params = Arrays.stream(args).map(o -> o == null ? "null" : o.getClass().getName().toString()).collect(Collectors.joining(","));
        throw new ModelException(String.format("模型[%s]找不到方法:%s($s)", getName(), method, params));
    }
     ...
     * @return
     */
    public Object invokeSupper(Class<?> current, Records rec, String method, Object[] args) {
        if (methods.containsKey(method)) {
            boolean found = false;
            for (MetaMethod metaMethod : methods.get(method)) {
                if (found) {
        ...
        String params = StringUtils.join(Arrays.stream(args).map(o -> o == null ? "null" : o.getClass().getName().toString()).toArray());
        throw new ModelException(String.format("模型[%s]找不到方法:%s($s)", getName(), method, params));
    }
    ...
    void autoInit(Environment env) {
        ...
        if (auto) {
            ...
            if (mustCreateTable) {
                String comment = getLabel();
                if (Utils.isNotEmpty(description)) {
                    comment += ": " + description;
                }
                sql.createModelTable(cr, getTable(), comment);
            }
            ...
            for (Entry<String, DbColumn> entry : columns.entrySet()) {
                MetaField field = fields.get(entry.getKey());
                DbColumn column = entry.getValue();
                boolean dropNotNull = (field == null || !field.isStore()) && !column.isNullable();
                if (dropNotNull) {
                    sql.dropNotNull(cr, getTable(), entry.getKey(), column.getDbType());
                }
            }
        }
    ...
    void setupBase(Environment env) {
        ...
        if (getPresent().length > 0) {
            for (String fname : getPresent()) {
                assert fields.containsKey(fname) : String.format("模型[%s]的present=%s无效", getName(), fname);
            }
        } else {
            if (fields.containsKey(Constants.CODE)) {
                if (fields.containsKey(Constants.NAME)) {
                    present = new String[]{Constants.CODE, Constants.NAME};
                    presentFormat = "{code}({name})";
                } else {
                    present = new String[]{Constants.CODE};
                }
        ...
        //设置授权关系
        MetaModel auth = this;
        String modelName = auth.getName();
        while (auth != null) {
            String authModel = auth.getAuthModel();
            if (Utils.isEmpty(authModel) || Utils.equals(authModel, auth.getName())) {
                break;
            }
            auth = env.getRegistry().getModels().get(authModel);
            if (auth != null) {
                auth.getAuthChildren().add(modelName);
            }
        }
    }
    ...
    void addMagicFields() {
        ...
        addField(Constants.PRESENT, Field.Char().label("呈现").search("searchPresent").compute("computePresent").automatic(true).setModule(module));
        // add display name
        if (isLogAccess()) {
            if (!fields.containsKey(Constants.CREATE_UID)) {
                addField(Constants.CREATE_UID, new Many2oneField(Constants.USER).label("创建人").automatic(true).readonly().setModule(module));
            }
            if (!fields.containsKey(Constants.CREATE_DATE)) {
                addField(Constants.CREATE_DATE, new DateTimeField().label("创建时间").automatic(true).readonly().setModule(module));
            }
            if (!fields.containsKey(Constants.UPDATE_UID)) {
                addField(Constants.UPDATE_UID, new Many2oneField(Constants.USER).label("修改人").automatic(true).readonly().setModule(module));
            }
            if (!fields.containsKey(Constants.UPDATE_DATE)) {
                addField(Constants.UPDATE_DATE, new DateTimeField().label("修改时间").automatic(true).readonly().setModule(module));
            }
        }
        // TODO last_modified
    }
     ...
     * @param field
     */
    void addField(String name, MetaField field) {
        fields.put(name, field);
        field.setName(this, name);
        boolean update = field.unique == Constants.UNIQUE_MODE.Update;
        if (update || field.unique == Constants.UNIQUE_MODE.Unique) {
            UniqueConstraint uc = new UniqueConstraint(name + "_unique", new String[]{name}, null, update, module);
            uniques.put(uc.getName(), uc);
        }
    }
