...
import org.jmaa.sdk.data.xml.parsing.XNode;
...
public class XMLScriptBuilder {
    ...
    public SqlTemplate parseScriptNode() {
        MixedSqlNode rootSqlNode = parseDynamicTags(context);
        SqlTemplate sqlTemplate;
        if (isDynamic) {
            sqlTemplate = new DynamicSqlTemplate(configuration, rootSqlNode);
        } else {
            sqlTemplate = new RawSqlTemplate(configuration, rootSqlNode);
        }
        return sqlTemplate;
    }
    ...
    protected MixedSqlNode parseDynamicTags(XNode node) {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (child.getNode().getNodeType() == Node.CDATA_SECTION_NODE || child.getNode().getNodeType() == Node.TEXT_NODE) {
                ...
                if (handler == null) {
                    throw new ValueException("Unknown element <" + nodeName + "> in SQL statement.");
                }
    ...
    private class ChooseHandler implements NodeHandler {
        ...
        private SqlNode getDefaultSqlNode(List<SqlNode> defaultSqlNodes) {
            ...
            if (defaultSqlNodes.size() == 1) {
                ...
                throw new ValueException("Too many default (otherwise) elements in choose statement.");
            }
