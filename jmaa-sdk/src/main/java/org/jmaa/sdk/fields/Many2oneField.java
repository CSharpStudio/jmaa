...
import org.jmaa.sdk.util.ToUpdate.IdValues;
...
public class Many2oneField extends RelationalField<Many2oneField> {
    ...
    public Many2oneField(String comodel, boolean delegate) {
        this(comodel);
     ...
     * 不生成外键约束
     */
    public Many2oneField ignoreFK(boolean ignoreFK) {
        args.put("ignoreFK", ignoreFK);
        return this;
    }
     ...
     * 不生成外键约束
     */
    public Many2oneField ignoreFK() {
        args.put("ignoreFK", true);
        return this;
    }
    ...
    protected void updateDbNotNull(Records model, DbColumn column) {
        if (column != null) {
            boolean hasNotNull = !column.isNullable();
            if (isRequired() && !hasNotNull) {
                Cursor cr = model.getEnv().getCursor();
                SqlDialect sd = cr.getSqlDialect();
                String fk = sd.limitIdentity(String.format("fk_%s_%s", model.getMeta().getTable(), getName()));
                cr.execute("SELECT count(id) FROM ir_model_constraint where name=%s", Arrays.asList(fk));
                if (ObjectUtils.toLong(cr.fetchOne()[0]) > 0) {
                    cr.execute("DELETE FROM ir_model_constraint where name=%s", Arrays.asList(fk));
                    sd.dropConstraint(cr, model.getMeta().getTable(), fk);
                }
            }
        }
        super.updateDbNotNull(model, column);
    }
    ...
    public Records write(Records records, Object value) {
        ...
        if (cacheValue != null) {
            Records corecord = (Records) convertToRecord(cacheValue, records);
            List<String> inverses = corecord.getMeta().getFieldInverses(getName());
            if (ObjectUtils.isNotEmpty(inverses)) {
                for (String inverse : inverses) {
                    MetaField inverseField = corecord.getMeta().getField(inverse);
                    cache.remove(corecord, inverseField);
                }
            }
        }
        return records;
