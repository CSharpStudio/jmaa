//@ sourceURL=AerialView.js
jmaa.component('AerialView', {
    width: 156,
    minimize: false,
    getTpl() {
        return `<span class="fas fa-angle-up minimize" role="button"></span>
    ...
    updateView() {
        ...
        let parentWidth = canvasContainer.width();
        let parentHeight = canvasContainer.height();
        let rate = canvasWidth > parentWidth ? me.width / canvasWidth : me.width / parentWidth;
        let viewWidth = canvasWidth > parentWidth ? rate * parentWidth : me.width;
        let viewHeight = canvasHeight > parentHeight ? viewWidth * parentHeight / parentWidth : rate * canvasHeight;
    ...
    load() {
        let me = this;
        me.thumbnail.empty();
        me.diagram.items.each(function () {
            me.add(this.id);
        });
    },
    update(id) {
