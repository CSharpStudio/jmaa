        ...
        let tpl = `
            <div class="color-box">
                <div class="color-item">
                    <div class="item-con" style="background-color: #dcdcdc"></div>
                    <div class="item-txt">过期</div>
                </div>
                <div class="color-item">
                    <div class="item-con" style="background-color: #edf2fd"></div>
                    <div class="item-txt">已排</div>
                </div>
                <div class="color-item">
                    <div class="item-con" style="background-color: #aaaaaa"></div>
                    <div class="item-txt">公休日</div>
                </div>
                <div class="color-item">
                    <div class="item-con" style="background-color: #f3ce98"></div>
                    <div class="item-txt">节假日</div>
                </div>
<!--                <div class="color-item">-->
<!--                    <div class="item-con" style="border:1px solid #0015ff;background-color: #fff"></div>-->
<!--                    <div class="item-txt">今天</div>-->
<!--                </div>-->
            </div>
            ...
            <div class="operate-box">
                <div class="sc-actions">
                 ...
                 <div class="sc-actions">
                  ...
                  <div class="radio-box">
                    <div class="radio-box-item radio-box-item-only radio-box-item-act">单选</div>
                    <div class="radio-box-item radio-box-item-all">全选</div>
                  </div>
            </div>
        `
      header.innerHTML = header.innerHTML + tpl;
      ...
      for (var i = 0; i < 35; i++) {
        days.innerHTML = days.innerHTML + '<div class="sc-item"><div class="day"></div><div class="lunar-day"></div><div class="mark-con"></div></div>';
      }
      //添加下拉框数据
    ...
    value: function addData(year, month) {
      ...
      for (var i = 0; i < 35; i++) {
        ...
        // 之前的日期都需要设置颜色
        //   今天
          let todayValue = new Date(`${this.tyear}-${this.tmonth}-${this.tday}`).valueOf()
          // 每天
          let dayValue = new Date(`${writeyear}-${writemonth}-${writeday}`).valueOf()
          // 添加时间过期样式
          if (todayValue>=dayValue) {
              daysElement[i].classList.add('sc-expiredDay');
          }
        //   添加周末样式
          if(dayIndex===6||dayIndex===0){
              daysElement[i].classList.add('sc-weekend');
      ...
      if (options.showMark) {
        ...
        if (data) {
          daysElement.forEach(function (v, i) {
            ...
            let targetDay = `${year}-${currentmonth<10?'0'+currentmonth:currentmonth}-${day<10?'0'+day:day}`
            let dayInfo = data[targetDay]
              v.setAttribute('date',targetDay)

            if (dayInfo) {
              v.classList.add('sc-mark');
              let markTpl = ''
                for (const markTplKey in dayInfo) {
                    let value = dayInfo[markTplKey].value?dayInfo[markTplKey].value:dayInfo[markTplKey]
                    if(Array.isArray(value)) value = value[1]
                    if(markTplKey!=='date_type') markTpl=markTpl+`<div class="mark-con-item" style="${dayInfo[markTplKey].style?dayInfo[markTplKey].style:''}">${value}</div>`
                }
                v.querySelector('.mark-con').innerHTML = markTpl;
            } else {
              v.classList.remove('sc-mark');
                v.querySelector('.mark-con').innerHTML =''
            }
          });
        }
      }
      ...
      if (options.showHoliday) {
        ...
        if (data) {
          daysElement.forEach(function (v, i) {
            ...
            if (day == 1) currentmonth++;
            //国际节日
              let targetDay = `${currentmonth<10?'0'+currentmonth:currentmonth}-${day<10?'0'+day:day}`
            if (data.indexOf(targetDay) > 0) {
    ...
    value: function updateEvent() {
      ...
      daysElement.forEach(function (v, i) {
        ...
        v.onclick = function () {
            // 当前是否已选中
            let isSelected = this.className.indexOf('sc-selected')>=0
            // 检查是否是过期的日子
            let isExpired = this.className.indexOf('sc-expiredDay')>=0
            let isWeekend = this.className.indexOf('sc-weekend')>=0
            let isVocation = this.className.indexOf('sc-vocation')>=0
            ...
            if (pre&&!calendar.isAll) pre.classList.remove('sc-selected');
                if(!isSelected){
                    calendar.selectDay = v;
                    this.classList.add('sc-selected');
                }else if(calendar.isAll&&isSelected) {
                    calendar.selectDay = '';
                    this.classList.remove('sc-selected');

                }
            if(options.onValueItemClick){
                let targetDay = this.getAttribute('date')
                let item =  data[targetDay] ||{}
                let dateType = isWeekend&&isVocation?'holiday':(isVocation?'holiday':(isWeekend?'public_holiday':'work'))
                options.onValueItemClick.call(options.me,{val:{ date:targetDay,dateType,...item }})
            }
      ...
      let onlySub = container.querySelector('.radio-box-item-only');
        let allSub = container.querySelector('.radio-box-item-all');
        // 单选方法
         function onlyFN(){
            calendar.isAll = false
            allSub.classList.remove('radio-box-item-act')
            onlySub.classList.add('radio-box-item-act')
            daysElement.forEach(function (v, i) {
                let pre = container.querySelector('.sc-selected');
                if (pre) pre.classList.remove('sc-selected');
            })
        ...
        // 单选点击事件
        onlySub.onclick = function (){
             onlyFN()
        }
        // 多选点击事件
        allSub.onclick = function (){
             let formValue = options.onAllClickGetFormValue?options.onAllClickGetFormValue.call(options.me):null
            if(formValue){
                calendar.isAll = true
                onlySub.classList.remove('radio-box-item-act')
                allSub.classList.add('radio-box-item-act')
                daysElement.forEach(function (v, i) {
                    let isNotExpired = v.className.indexOf('sc-expiredDay')===-1
                    let isNotWeekend = v.className.indexOf('sc-weekend')===-1
                  let isNotVocation = v.className.indexOf('sc-vocation')===-1

                  // 过期且周末且节假日不能选中
                    if(isNotExpired&&isNotWeekend&&isNotVocation) {
                        data[v.getAttribute('date')] =formValue
                        // calendar.selectDay = v;
                        calendar.addMark(data)
                    }
                })
            }
      ...
      //初始化标记
      key: 'initMark',
      value: function addMark(val) {
          let that  = this
          for (const valKey in val) {
              that._options.mark[valKey] = val[valKey];
          }
          this.update();
      }
      }
  , {
    ...
    value: function addMark(val) {
        ...
        let {selectYear,selectMonth} = this.getSelectedDay()
    ...
    value: function getSelectedDay() {
      ...
      let selectDay = this.container.querySelector('.day').innerHTML;  //我加的
...
SimpleCalendar.prototype.languageData = {
  ...
  vocation: {
    data_2023: ['01-01', '01-02','01-21','01-22','01-23','01-24','01-25','01-26','01-27','04-05','04-29','04-30','05-01','05-02','05-03','06-22','06-23','06-24','09-29','09-30','10-01','10-01','10-02','10-03','10-04','10-05','10-06']
  }
};
