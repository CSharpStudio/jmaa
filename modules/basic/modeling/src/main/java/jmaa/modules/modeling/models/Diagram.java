...
public class Diagram extends Model {
    ...
    @Model.ServiceMethod(auth = "read", ids = false)
    ...
    public String addModel(Records rec, Map<String, Object> values) {
        ...
        values.put("module_id", rec.getRec("module_id").getId());
    ...
    public Map<String, Object> linkModel(Records rec, String modelId, String x, String y) {
        ...
        List<String> fieldNames = fields.getMeta().getFields().values().stream().filter(f -> !f.isAuto())
            .map(f -> f.getName()).collect(Collectors.toList());
        result.put("x", x);
        result.put("y", y);
        result.put("fields", fields.read(fieldNames));
        return result;
    }
    ...
    @Model.ServiceMethod(auth = "update", ids = false)
    ...
    public List<Map<String, Object>> loadDiagram(Records rec) {
        rec.ensureOne();
        ...
        List<String> fieldNames = fields.getMeta().getFields().values().stream().filter(f -> !f.isAuto())
            .map(f -> f.getName()).collect(Collectors.toList());
        List<Map<String, Object>> fieldRows = fields.read(fieldNames);
        Map<String, List<Map<String, Object>>> modelField = new HashMap<>(fieldRows.size());
    ...
    public void reflect(Records rec) {
        rec.ensureOne();
        String moduleId = rec.getRec("module_id").getId();
        String pkg = (String) rec.getRec("module_id").get("package_info");
        if (Utils.isEmpty(pkg)) {
            throw new ValidationException(rec.l10n("模块%s未维护源码包", rec.getRec("module_id").get("name")));
        }
        ...
        for (Class<?> clazz : manifest.models()) {
            ...
            Records exists = model.find(Criteria.equal("model", name).and(Criteria.equal("module_id", moduleId)));
            boolean createShape = true;
            if (exists.any()) {
                updateModel(exists, meta, clazz);
                Records s = shape.find(Criteria.equal("model_id", exists.getId()).and(Criteria.equal("diagram_id", rec.getId())));
                if (s.any()) {
                    createShape = false;
                }
            } else {
    ...
    Records createModel(Records model, String name, String moduleId, Model.Meta meta, Class<?> clazz) {
        ...
        for (java.lang.reflect.Field f : fields) {
            if (Field.class.isAssignableFrom(f.getType()) && Modifier.isStatic(f.getModifiers())) {
                try {
    ...
    Records createField(Records field, String name, MetaField meta, String modelId) {
        Set<String> fieldNames = field.getMeta().getFields().values().stream().filter(f -> !f.isAuto())
            .map(f -> f.getName()).collect(Collectors.toSet());
        Map<String, Object> map = getFieldArgs(meta, fieldNames);
        map.put("name", name);
    ...
    @SuppressWarnings("unchecked")
    Map<String, Object> getFieldArgs(MetaField meta, Set<String> fieldNames) {
