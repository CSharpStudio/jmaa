jmaa.component("Modeling", {
    ...
    init: function () {
        ...
        me.ws = me.dom.find('.main-box').JTabs({
            tabActived: function (e, tabs, tab) {
                ...
                let hash = jmaa.web.getParams(window.location.hash.substring(1));
                hash.diagram = id;
                window.location.hash = $.param(hash);
                let tophash = jmaa.web.getParams(top.window.location.hash.substring(1));
                if (tophash.u) {
        ...
        me.modelEditor = jmaa.create("FormEdit", {model: 'modeling.model', title: '模型'.t()});
        me.fieldEditor = jmaa.create("FormEdit", {model: 'modeling.model.field', title: '字段'.t()});
        me.modules = new ModuleView({
            dom: me.dom.find('.left-box'),
            designer: me,
            model: me.model,
            module: me.module
        });
    ...
    openDiagram: function (node) {
        ...
        me.ws.openTab(node.id, {
            ...
            closable: true,
            init: function (tab) {
                me.diagrams[node.id] = new Diagram({
                    dom: tab,
                    id: node.id,
...
$(function () {
    new Modeling({dom: $('.model-designer')});
});
