...
@Model.Meta(inherit = "mfg.material_return_mobile", label = "移动端退料")
...
public class MaterialReturnMobile extends ValueModel {
    static Field work_order_id = Field.Many2one("mfg.work_order").label("工单").store(false);
    static Field material_id = Field.Many2one("md.material").label("物料").store(false);
    ...
    public Records createBatch(Records rec, List<Map<String, Object>> valuesList) {
        return rec.getEnv().get("mfg.material_return").createBatch(valuesList);
    }
