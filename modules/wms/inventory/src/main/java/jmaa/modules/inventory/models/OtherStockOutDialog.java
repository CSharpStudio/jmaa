...
@Model.Meta(name = "wms.other_stock_out_dialog", label = "其它出库-弹框", authModel = "wms.other_stock_out")
public class OtherStockOutDialog extends ValueModel {
    ...
    static Field print_flag = Field.Boolean().label("打印新标签").defaultValue(false);
    ...
    public Action onWarehouseChange(Records record) {
        ...
        if (warehouse.any() && material.any()) {
            onhand = (Records) record.getEnv().get("wms.other_stock_out").call("findOnhandByRule", material, warehouse);
        }
