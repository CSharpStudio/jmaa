//@ sourceURL=inventory_check_second_mobile.js
jmaa.view({
    init() {
        let me = this;
    ...
    loadOrderList(list, callback) {
        let me = this;
        let criteria = [["status", "in", ["second_running", "first_done", "second_done"]],["second_flag","=",true]];
    ...
    openDetails() {
        ...
        if ('second_done' === me.orderList.getSelectedData()[0].status[0] || 'second_done' === me.orderList.getSelectedData()[0].status[0]) {
            //me.dom.find('.second-check-final .scan-confirm').hide();
            me.dom.find('.second-check-final').hide();
            me.dom.find('.scan-confirm').hide();
        } else {
            //me.dom.find('.second-check-final .scan-confirm').show();
            me.dom.find('.second-check-final').show();
    ...
    loadLineList(list, callback) {
        ...
        if (scope == 'undone') {
            criteria.push(['status', '=', 'first_done']);
        }
        jmaa.rpc({
            model: me.model,
            module: me.module,
    ...
    loadDetailsList(list, callback) {
        ...
        if (scope == 'undone') {
            criteria.push(['status', '!=', 'second_done']);
        }
        jmaa.rpc({
            model: me.model,
            module: me.module,
    ...
    scanCode() {
        ...
        jmaa.rpc({
            ...
            onsuccess: function (r) {
                ...
                me.scanMaterialForm.editors.second_qty.setValue(r.data.data.second_qty);
    ...
    scanConfirm: function () {
        ...
        let secondQty = form.editors.second_qty.getRawValue();
        if (!secondQty || secondQty < 0) {
        ...
        jmaa.rpc({
            ...
            onsuccess: function (r) {
                ...
                me.scanMaterialForm.editors.second_qty.setValue('');
